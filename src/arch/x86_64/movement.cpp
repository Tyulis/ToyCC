...
namespace toycc::arch::x86_64 {
    ...
    static std::string get_mov_mnemonic(size_t size, CodeLocation code_location) {
        auto it = MOV_MNEMONIC.find(size);
        if (it == MOV_MNEMONIC.end())
            throw Diagnostic(DiagnosticLevel::INTERNAL_ERROR, std::format("No `mov` instruction for size {}", size), code_location);
        return it->second;
    }

    std::shared_ptr<Declaration> CodeGenerator::load_constant(StackFrame& frame, const Constant& constant, LOC destination, CodeLocation code_location) {
        ...
        code << get_mov_mnemonic(size, code_location) << " $" << constant.integer() << ", " << destination_ref.value();
    ...
    void CodeGenerator::move_variable(StackFrame& frame, std::shared_ptr<Declaration> variable, LOC destination, CodeLocation code_location) {
        ...
        std::stringstream code;
        code << get_mov_mnemonic(size, code_location) << " " << source_ref << ", " << destination_ref;
        frame.output.statement(code.str());
