...
namespace toycc::arch::x86_64 {
    enum IdentifierType {
        ...
        DEREFERENCE,  // Dereferences something
    ...
    static inline IdentifierType indirect_identifier_type(ir::TypeCategory category) {
        switch (category) {
    ...
    std::ostream& operator<< (std::ostream& stream, const StatementOperandIdentifier& id) {
        switch (id.type) {
            ...
            case IdentifierType::DEREFERENCE: stream << "deref ";  break;
    ...
    static void set_operand_match(StatementMatch& match, const StatementOperandIdentifier& id, const OperandMatch& operand_match) {
        switch (id.type) {
            ...
            case IdentifierType::DEREFERENCE:
                if (id.is_input)  match.input[id.index] = operand_match.with_index(match.input[id.index].input_index);
                else              match.output = operand_match.with_index(match.output->input_index);
                break;
    ...
    static ir::Operand get_operand(const StatementMatch& statement_match, const ir::Statement& statement, const StatementOperandIdentifier& id) {
        ...
        switch (id.type) {
            ...
            default: throw Diagnostic(DiagnosticLevel::INTERNAL_ERROR, "Unknown identifier type");
        }
    ...
    static bool is_output_operand(const TranslationOperandIdentifier& id) {
        if (std::holds_alternative<TranslationOperandIdentifier::Allocation>(id.id))
            return false;
        const TranslationOperandIdentifier::Statement& statement_id = std::get<TranslationOperandIdentifier::Statement>(id.id);
        return !statement_id.id.is_input && (statement_id.id.type == IdentifierType::DIRECT || statement_id.id.type == IdentifierType::DEREFERENCE);
    }
        ...
        for (const auto& [value, row] : weights.value_rows) {
            ...
            for (const TranslationOperandIdentifier& id : value.operands) {
                switch (id.type()) {
                    ...
                    case IdentifierType::DEREFERENCE: allowed_locations = unordered_set_intersection(allowed_locations, get_operand_match(match, id).locations);  break;
